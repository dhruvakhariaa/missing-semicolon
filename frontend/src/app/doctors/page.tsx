...
const DoctorsContent = () => {
    ...
    const router = useRouter(); // To update URL params if needed, or we can use Link
    const deptFilter = searchParams.get('dept');

    // We can use router.push to update params, but for local state filters we might just keep them local or update URL.
    // The original code used local state for most filters and useSearchParams only for dept initialization.
    // But `setSearchParams` was used to clear ALL filters. 
    // In Next.js, updating search params triggers navigation.
    // For faithful reproduction, let's stick to local state mostly, but handle the Clear All.
    ...
    // Sync selectedDept with URL param if it changes externally or initially
    useEffect(() => {
        if (deptFilter) {
    ...
    const clearFilters = () => {
        ...
        setFeeFilter('');
        setRatingFilter('');
        router.push('/doctors'); // Clear URL params
    };
...
export default function Doctors() {
    return (
        <Suspense fallback={<div className="p-8 text-center text-gov-blue-600">Loading...</div>}>
            <DoctorsContent />
        </Suspense>
    );
}
