...
export default function ComplaintList() {
    ...
    const fetchComplaints = async () => {
        ...
        setComplaints([
            { _id: '1', title: 'Water Leakage', description: 'Severe pipe leakage on Main St.', category: 'Water', location: 'Sector 4, Main Street', status: 'In Progress', createdAt: new Date().toISOString() },
            { _id: '2', title: 'Streetlight Broken', description: 'Streetlight #45 is flickering.', category: 'Electricity', location: 'Park Avenue', status: 'Pending', createdAt: new Date().toISOString() },
            { _id: '3', title: 'Garbage Dump', description: 'Garbage not collected for 3 days.', category: 'Waste Management', location: 'Block C, Market', status: 'Resolved', createdAt: new Date().toISOString() }
        ]);
        setLoading(false);
    };
    ...
    return (
        <div className="space-y-4">
            ...
            {complaints.map((complaint) => (
                <div key={complaint._id} className="bg-white p-5 rounded-xl shadow-sm border border-brand-50 hover:border-brand-200 hover:shadow-md transition-all duration-300 group">
                    ...
                    <h4 className="font-bold text-gray-800 text-lg mb-2 group-hover:text-brand-600 transition-colors">{complaint.title}</h4>
